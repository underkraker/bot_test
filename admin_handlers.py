...
def register_admin(bot):
    ...
    def adm_actions(c):
        ...
        # SECCIÓN DE ELIMINAR CON LISTA DE USUARIOS
    ...
    # MANEJADORES DE CONFIRMACIÓN Y EJECUCIÓN
    ...
    def ejecutar_del(c):
        ...
        bot.answer_callback_query(c.id, f"✅ {user} eliminado")
    ...
    # PASOS DE RESELLER CON BOTÓN CANCELAR
    def step_add_res(m, bot):
        if m.text == "/start" or m.text == "❌ Cancelar": 
            bot.send_message(m.chat.id, "🚫 Cancelado.", reply_markup=keyboards.menu_admin())
            return
        try:
